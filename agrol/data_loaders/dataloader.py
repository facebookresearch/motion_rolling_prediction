...
def get_manifold_paths(dataset_path, split):
    ...
    for d in parent_data_path:
        if pathmgr.isdir(os.path.join(dataset_path, d)):
            files = [
...
def load_data_from_manifold(dataset, dataset_path, split, **kwargs):
    """
    ...
    if split == "test":
        ...
        # motion_list = [torch.load(pathmgr.get_local_path(i)) for i in tqdm(motion_list)]
        with mp.Pool(8) as p:
            local_paths = list(
                tqdm(
                    p.imap(pathmgr.get_local_path, motion_list), total=len(motion_list)
                )
            )
            motion_list = [torch.load(i) for i in tqdm(local_paths)]
