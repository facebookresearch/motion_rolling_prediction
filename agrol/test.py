...
def non_overlapping_test(
    ...
    if (
        args.input_motion_length <= num_frames
    ):  # we split the sequence into non-overlapping sequences of 196 frames
        while count < num_frames:
            if (
                count + args.input_motion_length > num_frames
    ...
    if args.vis or args.vis_gt:
        ...
        with tempfile.TemporaryDirectory() as tmpdirname:
            ...
            if args.vis and not pathmgr.exists(save_video_path):
                ...
                pathmgr.copy(
                    src_path=temp_video_path, dst_path=save_video_path, overwrite=True
                )

            # Save GT
            save_video_path_gt = os.path.join(video_dir, save_filename + "_gt.mp4")
            if args.vis_gt and not pathmgr.exists(save_video_path_gt):
                temp_video_path_gt = os.path.join(tmpdirname, save_filename + "_gt.mp4")
                utils_visualize.save_animation(
                    body_pose=gt_body,
                    savepath=temp_video_path_gt,
                    bm=body_model.body_model,
                    fps=fps,
                    resolution=(800, 800),
                )
                pathmgr.copy(
                    src_path=temp_video_path_gt,
                    dst_path=save_video_path_gt,
                    overwrite=False,
                )
...
def main():
    ...
    if model_type == "diffusion":
        ...
        sample_fn = diffusion.ddim_sample_loop if args.ddim else diffusion.p_sample_loop
        logger.info(f"Using sampler: {sample_fn=}")
    elif model_type == "mlp":
    ...
    for i in tqdm(range(len(dataset))):
        ...
        all_results.append([filename, num_frames] + list(instance_log.values()))

    df = pd.DataFrame(
        all_results, columns=["filename", "num_frames"] + list(instance_log.keys())
    )

    pathmgr.mkdirs(args.output_dir)
    csv_path = os.path.join(args.output_dir, f"results_{args.dataset}.csv")
    # save to temporary file
    with tempfile.TemporaryDirectory() as tmpdirname:
        csv_path = os.path.join(tmpdirname, f"results_{args.dataset}.csv")
        df.to_csv(csv_path, index=False)
        # copy to manifold
        pathmgr.copy(
            src_path=csv_path,
            dst_path=os.path.join(args.output_dir, f"results_{args.dataset}.csv"),
            overwrite=True,
        )
    ...
    for metric in pred_metrics:
        logger.info(f"{log[metric] / len(dataset) * metrics_coeffs[metric]}")
    logger.info("Metrics for the ground truth")
    for metric in gt_metrics:
        logger.info(f"{metric, log[metric] / len(dataset) * metrics_coeffs[metric]}")
