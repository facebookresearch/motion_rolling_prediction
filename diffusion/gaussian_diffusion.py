...
from utils.constants import DiffusionLossWeightType, NoiseScheduleType
...
def get_named_beta_schedule(
    schedule_name: NoiseScheduleType,
    ...
    """
    if schedule_name == NoiseScheduleType.LINEAR:
    ...
    elif schedule_name == NoiseScheduleType.COSINE:
        return betas_for_alpha_bar(
            num_diffusion_timesteps,
            lambda t: math.cos((t + 0.008) / 1.008 * math.pi / 2) ** 2,
        )
    elif schedule_name == NoiseScheduleType.COSINE_NOEXP:
        return betas_for_alpha_bar(
            num_diffusion_timesteps,
...
class GaussianDiffusion:
    ...
    def __init__(
        ...
        **kwargs,
        ...
        def get_sigma_from_betas(num_timesteps, fn):
            betas = betas_for_alpha_bar(num_timesteps, fn)
            alphas = 1.0 - betas
        ...
        self.mse_loss_weight_type, self.mse_loss_weight_suffix = (
            DiffusionLossWeightType.parse(mse_loss_weight_type)
        )
    ...
    def q_sample(self, x_start, t, noise=None, clamp_noise=False):
        """
        ...
        :param clamp_noise: clamp the noise at [-1, 1].
        ...
        alpha = self.get_alpha_from_t(t, x_start.shape)
        sigma = self.get_sigma_from_t(t, x_start.shape)
    ...
    def p_sample_loop(
        ...
        dump_steps=None,
        const_noise=False,
        deactivate_last_inpainting=False,
    ):
        """
        ...
        :param dump_steps: If True, will return a list of samples at all timesteps, if [1, 2, 3] it will return samples at those timesteps
        :param deactivate_last_inpainting: If True, it won't fill the context after predicting the last denoising iteration (it will return what the model predicts)
        ...
        for i, sample in enumerate(
            ...
            if dump_steps is not None and (dump_steps or i in dump_steps):
    ...
    def p_sample_loop_progressive(
        ...
        for i in indices:
            t = self.sample_t(
                i, shape[0], shape[1], device=device
            )  # th.tensor([i] * shape[0], device=device)
            deactivate_inpainting = (
                i == indices[-1]
                and deactivate_last_inpainting
                and model_kwargs is not None
                and "y" in model_kwargs
                and "inpainting_mask" in model_kwargs["y"]
            )
            if deactivate_inpainting:
                inpainting_mask = model_kwargs["y"]["inpainting_mask"]
    ...
    def ddim_sample(
        ...
        nonzero_mask = (
            (t != 0).float().view(*t.shape, *([1] * (len(x.shape) - len(t.shape))))
        )  # no noise when t == 0
        sample = mean_pred + nonzero_mask * sigma * noise
        out_orig["sample"] = sample
        return out_orig
    ...
    def _prior_bpd(self, x_start):
        ...
        t = self.sample_t(
            self.num_timesteps - 1, batch_size, seq_len, device=x_start.device
        )  # th.tensor([self.num_timesteps - 1] * batch_size, device=x_start.device)
    ...
    def sample_t(self, index, batch_size, seq_len, device="cpu"):
        return th.tensor([index] * batch_size, device=device)

    def get_sigma_from_t(self, t, out_shape):
        """
        Get the sigma from the timesteps.
        """
        return _extract_into_tensor(self.sqrt_one_minus_alphas_cumprod, t, out_shape)

    def get_alpha_from_t(self, t, out_shape):
        """
        Get the alpha from the timesteps.
        """
        return _extract_into_tensor(self.sqrt_alphas_cumprod, t, out_shape)

    def get_snr_from_t(self, t, out_shape):
        """
        Get the signal-to-noise ratio from the timesteps.
        """
        return (
            self.get_alpha_from_t(t, out_shape) / self.get_sigma_from_t(t, out_shape)
        ) ** 2

    def get_loss_weights(self, t):
        """
        Implementation of Min-SNR-5 diffusion loss. It is a weighting strategy to increase the importance of the
        denoising steps with higher SNR, but with an upper bound (set as parameter, but 5 is reported as a good value).
        It reportedly improves the convergence speed and final quality of the results. Unexplored for Rolling Diffusion.
        Paper: https://openaccess.thecvf.com/content/ICCV2023/papers/Hang_Efficient_Diffusion_Training_via_Min-SNR_Weighting_Strategy_ICCV_2023_paper.pdf
        Code from: https://github.com/TiankaiHang/Min-SNR-Diffusion-Training/blob/main/guided_diffusion/gaussian_diffusion.py
        """
        mse_loss_weight = None
        snr = self.get_snr_from_t(t, t.shape)

        k = self.mse_loss_weight_suffix
        # get loss weight
        if (
            self.model_mean_type is not ModelMeanType.START_X
            or self.mse_loss_weight_type == DiffusionLossWeightType.CONSTANT
        ):
            mse_loss_weight = th.ones_like(t)
            if self.mse_loss_weight_type == DiffusionLossWeightType.MIN_SNR:
                # min{snr, k}
                mse_loss_weight = (
                    th.stack([snr, k * th.ones_like(t)], dim=1).min(dim=1)[0] / snr
                )

            elif self.mse_loss_weight_type == DiffusionLossWeightType.VMIN_SNR:
                # min{snr, k}
                mse_loss_weight = th.stack([snr, k * th.ones_like(t)], dim=1).min(
                    dim=1
                )[0] / (snr + 1)

            elif self.mse_loss_weight_type == DiffusionLossWeightType.MAX_SNR:
                # max{snr, k}
                mse_loss_weight = (
                    th.stack([snr, k * th.ones_like(t)], dim=1).max(dim=1)[0] / snr
                )

        else:
            if self.mse_loss_weight_type == DiffusionLossWeightType.TRUNC_SNR:
                # max{snr, 1}
                mse_loss_weight = th.stack([snr, th.ones_like(t)], dim=1).max(dim=1)[0]
            elif self.mse_loss_weight_type == DiffusionLossWeightType.SNR:
                mse_loss_weight = snr

            elif self.mse_loss_weight_type == DiffusionLossWeightType.INV_SNR:
                mse_loss_weight = 1.0 / snr

            elif self.mse_loss_weight_type == DiffusionLossWeightType.MIN_SNR:
                # min{snr, k}
                mse_loss_weight = th.stack([snr, k * th.ones_like(t)], dim=1).min(
                    dim=1
                )[0]
                ...
                # min{snr, k}
                mse_loss_weight = th.stack([snr, k * th.ones_like(t)], dim=1).max(
                    dim=1
                )[0]

        if mse_loss_weight is None:
            raise ValueError("mse loss weight is not correctly set!")
