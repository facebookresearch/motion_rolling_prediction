...
def save_animation_single(vertices, faces, resolution=(800, 800)):
    imw, imh = resolution
    mv = MeshViewer(width=imw, height=imh, use_offscreen=True)
...
def save_animation_mp(body_pose, savepath, bm, fps=60, resolution=(800, 800)):
    faces = c2c(bm.f)
    img_array = []
    f = partial(save_animation_single, faces=faces, resolution=resolution)

    with mp.Pool(8) as p:
        img_array = list(tqdm(p.imap(f, body_pose.v), total=body_pose.v.shape[0]))
    out = cv2.VideoWriter(savepath, cv2.VideoWriter_fourcc(*"mp4v"), fps, resolution)
